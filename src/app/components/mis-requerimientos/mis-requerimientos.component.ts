  ...
  ngOnInit(): void {
    ...
    if (legajo) {
      this.requerimientosService
        .getRequerimientoByIdDocente(+legajo)
        .subscribe((requerimientos) => {
          console.log(requerimientos);
          if (requerimientos) {
            this.estadoRequerimientos =
              this.route.snapshot.paramMap.get('estado') || '';
            if (this.estadoRequerimientos == 'Pendiente') {
              this.requerimientos = requerimientos.filter(
                (requerimiento) =>
                  requerimiento.estado === this.estadoRequerimientos ||
                  requerimiento.estado == 'Atendido',
              );
            } else {
              this.requerimientos = requerimientos.filter(
                (requerimiento) =>
    ...
    modalRef.result.then((result) => {
      if (result === 'Eliminar') {
        if (requerimiento.idRequerimiento) {
          this.requerimientosService
            .eliminarRequerimiento(requerimiento.idRequerimiento)
            .subscribe(
              (res) => {
                this.ngOnInit();
              },
